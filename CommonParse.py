...
class BytesStream(object):
    ...
    def set_segment_by_index(self, value=int, idx_start=int, val_width=int):
        ...
        if val_width < 1:
            ...
            if len(ret_bytes_array) > byte_offset_start:
                pass
            else:
                ret_bytes_array += (b'\x00'*(byte_offset_start - len(ret_bytes_array) + 1))  # if just right not bit left, append one byte
            # process joint
            ...
            # make up hex string head,cpmpute num '0'
            str_value_hex = '0'*(int((val_width+l_offet + 7)/8)*2 - len(str_value_hex)) + str_value_hex
    ...
    def fast_set_segment_by_index(self, value=int, idx_start=int, val_width=int):
        ...
        if val_width < 1:
            ...
            if self.hexStream != '':
                ...
                bin_stream_str = '0'*(len(self.hexStream)*4 - len(bin_stream_str)) + bin_stream_str
            else:
                bin_stream_str = ''
            ...
            if (val_width / 8) > 1 and self.endian == 1:    # if little endian, slip,reverse and assemble the new value
                ...
                if val_width % 8 == 0:
                    ...
                    raise BytesEndianException()
            # assemble bin string
            if len(bin_stream_str) > idx_start:
                bin_stream_str = bin_stream_str[:idx_start] + bin_value_str
            else:
                bin_stream_str = bin_stream_str + '0'*(idx_start - len(bin_stream_str)) + bin_value_str
            ...
            hex_stream_str = '0'*(int(len(bin_stream_str)/8)*2 - len(hex_stream_str)) + hex_stream_str
            # update hex stream
            self.hexStream = hex_stream_str
